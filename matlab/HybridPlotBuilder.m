...
%   runtests('hybrid.tests.slow_essential.HybridPlotBuilderTest')
    ...
    methods % Property setters for plot settings.
        ...
        function this = addLegendEntry(this, graphic_obj_handle, label, data_struct)
            ...
            this.data_for_legend{end+1} = data_struct;
            this.display_legend();
            if nargout == 0
                % Prevent output if function is not terminated with a semicolon.
                clearvars this
    ...
    methods(Access = private)
        ...
        function plot_from_plot_data_array(this, plot_data_array)
            ...
            for plt_data = plot_data_array
                ...
                % data_struct = struct(plt_data.flow_args{:})
                this.addLegendEntry(dummy_plt, plt_data.legend_label, plt_data);
        ...
        function display_legend(this)
            ...
            data_for_this_legend = this.data_for_legend(plots_for_legend_indices);
            if isempty(axes_for_this_legend) 
                ...
                % ax=[], leading to errors. This return statement prevents that.
                return 
            end
            for ax = unique(axes_for_this_legend)
                ...
                if isvalid(ax) % Check that figure hasn't been closed.
                    ...
                    for k = 1:numel(data_for_this_legend)
                        ...
                        % The elements in 'icon' are organizaed as follows:
                        % * First n_lgd_entries: text labels
                        % * Thereafter, pairs of two line objects. 
                        icon_indices = n_lgd_entries + 2*(k-1) + [1 2];
                        flow_line_and_start_jump_icon = icons(icon_indices(1));
                        jump_line_and_end_jump_icon = icons(icon_indices(2));

                        % Set the line properties of flow_line_and_start_jump_icon
                        % using the flow_args from data.
                        set(flow_line_and_start_jump_icon, data.flow_args{:});

                        % Set the marker properties of flow_line_and_start_jump_icon
                        % using the flow_args from data.
                        set(flow_line_and_start_jump_icon, data.jump_args.start{:});

                        % Set the line properties of flow_line_and_start_jump_icon
                        % using the flow_args from data.
                        set(jump_line_and_end_jump_icon, data.jump_args.line{:});

                        % Set the marker properties of flow_line_and_start_jump_icon
                        % using the flow_args from data.
                        set(jump_line_and_end_jump_icon, data.jump_args.end{:});
                        ...
                        % Only plot marker for second index because we are plotting the end of jump marker.
                        jump_line_and_end_jump_icon.MarkerIndices = 2;
                    end
