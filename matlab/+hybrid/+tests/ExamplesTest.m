classdef ExamplesTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function test_analog_to_digital_converter(~)
            clear
            checkExample('analog_to_digital_converter')
        end

        function test_behavior_in_C_intersection_D(~)
            clear
            checkExample('behavior_in_C_intersection_D')
        end
        ...
        function test_bouncing_ball_with_adc(~)
            clear
            checkExample('bouncing_ball_with_adc')
        end

        function test_bouncing_ball_with_input(~)
            clear
            checkExample('bouncing_ball_with_input')
        end

        function test_bouncing_ball_with_input_alternative(~)
            clear
            checkExample('bouncing_ball_with_input', 'bouncing_ball_with__input_alternative')
        end

        function test_coupled_subsystems(~)
            clear
            checkExample('coupled_subsystems')
        end

        function test_finite_state_machine(~)
            clear
            checkExample('finite_state_machine')
        end
        ...
        function test_network_estimation1(~)
            clear
            checkExample('network_estimation')
        end

        function test_network_estimation2(~)
            clear
            checkExample('network_estimation', 'network_estimation_with_input')
        end

        function test_vehicle_on_constrained_path(~)
            clear
            checkExample('vehicle_on_constrained_path')
        end

        function test_zero_order_hold(~)
            clear
            checkExample('zero_order_hold')
        end

        function test_zoh_feedback_control(~)
            clear
            checkExample('zoh_feedback_control')
        end
    end
end

function checkExample(example_name, model_name)
    if ~exist('optional_model_name', 'var')
