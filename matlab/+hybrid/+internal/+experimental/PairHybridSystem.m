...
%%% HybridSubsystem with inputs generated by kappa_*C and kappa_*D, 
...
%%% times).
    ...
    properties(GetAccess = private, SetAccess = immutable)
        subsystem1 HybridSubsystem = DummyHybridSubsystem()
        subsystem2 HybridSubsystem = DummyHybridSubsystem()
    ...
    methods(Sealed)
        function xdot = flowMap(this, x, t) 
            ...
            x1dot = this.subsystem1.flowMap(x1, this.kappa_1C(x1, x2), t, j1);
            x2dot = this.subsystem2.flowMap(x2, this.kappa_2C(x1, x2), t, j2);
            xdot = [x1dot; x2dot; 0; 0];
        end 

        function xplus = jumpMap(this, x, t)  
            [x1, x2, j1, j2] = this.split(x);
            D1 = this.subsystem1.jumpSetIndicator(x1, this.kappa_1D(x1, x2), t, j1);
            D2 = this.subsystem2.jumpSetIndicator(x2, this.kappa_2D(x1, x2), t, j2);
            if D1
                % If the state is in D1 (possibly in D1 union D2), then a jump
                % given by the subsystem1 jump map occurs.
                x1plus = this.subsystem1.jumpMap(x1, this.kappa_1D(x1, x2), t, j1);
                x2plus = x2;
                j1plus = j1 + 1;
                j2plus = j2;
            elseif D2
                ...
                % jump map occurs.
                x1plus = x1;
                x2plus = this.subsystem2.jumpMap(x2, this.kappa_2D(x1, x2), t, j2);
                j1plus = j1;
                j2plus = j2 + 1;
            else
                % warning('jumpMap called for x not in the jump set of either system')
                % We check that the jump maps can be evaluated.
                this.subsystem1.jumpMap(x1, this.kappa_1D(x1, x2), t, j1);
                this.subsystem2.jumpMap(x2, this.kappa_2D(x1, x2), t, j2);
                xplus = x;
                return
            end
        ...
        function C = flowSetIndicator(this, x, t) 
            [x1, x2, j1, j2] = this.split(x);
            C1 = this.subsystem1.flowSetIndicator(x1, this.kappa_1C(x1, x2), t, j1);
            C2 = this.subsystem2.flowSetIndicator(x2, this.kappa_2C(x1, x2), t, j2);
    ...
    methods(Access = protected)
        ...
        function [sol, ss1_sol, ss2_sol] = wrap_solution(this, t, j, x, tspan, jspan)
            ...
            sol = hybrid.CompositeHybridSolution(sol, ss_sols, tspan, jspan);
        end
    end
    ...
    methods(Access = private)
        ...
        function ndx = subsys_arg_to_ndx(this, subsys)
            ...
            elseif isnumeric(subsys) 
                ...
                error('Arguement must be either a HybridSubsystem object or a integer');
            end
        end
