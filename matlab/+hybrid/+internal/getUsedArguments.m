...
if ~isa(fh, 'function_handle')
    error('Hybrid:InvalidArgument', 'Argument was not a function handle, it was a %s', ...
        class(fh));
end

tokens = regexp(func2str(fh), '@\((?<args>.*?)\)','names');
if nargin(fh) ~= 0 && isempty(tokens)
    error('Hybrid:UnsupportedFunctionType','Function arguments for ''%s'' cannot be read. Use an anonymous function (such as @(x, t, j) x).', func2str(fh))
end
