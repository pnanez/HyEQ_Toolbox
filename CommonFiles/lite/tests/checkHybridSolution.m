function checkHybridSolution(sol, priority)
    ...
    verifyHybridSolutionDomain(sol.t, sol.j, sol.C_vals, sol.D_vals, priority)
    ...
    if ~(all(size(sol.x) == size(sol.f_vals)))
        error("HybridSolution:SizesDoNotMatch", ...
            "size(x)=%s does not match size(f_vals)=%s", ...
            mat2str(size(sol.x)), mat2str(size(sol.f_vals)))
    end 
    if ~(all(size(sol.x) == size(sol.g_vals)))
        error("HybridSolution:SizesDoNotMatch",  ...
            "size(x)=%s does not match size(g_vals)=%s", ...
            mat2str(size(sol.x)), mat2str(size(sol.g_vals)))
    end
