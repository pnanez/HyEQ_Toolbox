classdef PlotSettingsTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testErrorSetNonexistentProperty(testCase)
            ...
            no_property_fh = @() PlotSettings().set('not a property', 45);
            testCase.verifyError(no_property_fh, 'MATLAB:noPublicFieldForClass');            
        end
        
        function testErrorMissingValue(testCase)
            import hybrid.internal.*
            no_property_fh = @() PlotSettings().set('line width');
            testCase.verifyError(no_property_fh, 'HybridPlotBuiderDefaults:MissingValue');            
        end
        
        function testErrorNameNotString(testCase)
            import hybrid.internal.*
            no_property_fh = @() PlotSettings().set('text scale', 3, 5, 6);
            testCase.verifyError(no_property_fh, 'HybridPlotBuiderDefaults:NameNotString');            
        end
